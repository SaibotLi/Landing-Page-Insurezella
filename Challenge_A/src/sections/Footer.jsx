...
const Footer = () => {
  return (
    <footer className="w-full">
      {/* First container */}
      <div className="lg:w-4/5 lg:flex lg:mx-auto lg:justify-around w-11/12 grid grid-cols-2 justify-items-start border-2 shadow-md translate-y-10 bg-white shadow-gray-700 py-6">
        {/* Logo, span, icons */}
        ...
        <div className="flex-col justify-self-end mx-4">
          {/* H2, column with <img> icon + <span> */}
          <h2 className="text-blue-900 font-semibold text-2xl">Services</h2>
          <div className="flex-col space-y-2 lg:space-y-4 mt-4 mr-10">
            {insuranceTypes.map((insurance, index) => {
              const Icon = insurance.Icon;
              return (
                <div key={index} className="flex space-x-2 items-center">
                  <Icon className="w-6 h-6" />
                  <span className="text-gray-600">{insurance.label}</span>
                </div>
              );
            })}
          </div>
      ...
      {/* Box Divider */}
      <div className="lg:hidden w-full h-32 bg-indigo-500">Box</div>
      {/* Second container */}
